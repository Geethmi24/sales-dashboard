"""
Universal Sales Intelligence Hub — v11.0
Works with ANY Excel file sharing the same format (MEGA DERMA, HIMALAYA, etc.)
Key fix: dynamic TOTAL row detection + dynamic product row range
"""
...
# ══════════════════════════════════════════════════════
# EXCEL PARSING — UNIVERSAL (dynamic TOTAL row)
# ══════════════════════════════════════════════════════
def find_total_row(raw):
    """
    Dynamically find the TOTAL row that contains the LKR summary.
    Searches for a row where col B == 'TOTAL' and col E is a large number (>10000).
    Works regardless of how many product rows exist (11 for MEGA DERMA, 16 for HIMALAYA, etc.)
    """
    for i in range(5, min(80, raw.shape[0])):
        ...
        if val_b == 'TOTAL' and pd.notna(val_e) and isinstance(val_e, (int, float)) and float(val_e) > 10000:
            return i
    return None
...
def find_product_rows(raw, total_row):
    ...
    A valid product row has: a string product name in col B, and TAR > 0 in col E.
    """
    prod_rows = []
...
def parse_excel(file_obj):
    ...
    for sheet in sheets:
        ...
        if lkr_row_idx is None:
            continue  # Can't parse this sheet without a TOTAL row

        # ── Find product rows dynamically ───────────────────────
        ...
        # ── Collect product prices from col D ───────────────────
        for r in prod_rows:
        ...
        # ── LKR data from the dynamic TOTAL row ─────────────────
        lkr_ms = {}
        for ename, tc in entity_cols.items():
            ac = tc + 1
        ...
        # ── F-column % data (Sales Performance section) ─────────
        # Looks for "PRODUCT" header row after the TOTAL row, then reads B+F columns
        f_pct_sheet = {}
        perf_hdr_row = None
        for r in range(lkr_row_idx + 1, min(lkr_row_idx + 60, raw.shape[0])):
            val = str(raw.iloc[r, 1]).strip() if pd.notna(raw.iloc[r, 1]) else ''
            if val == 'PRODUCT':
                perf_hdr_row = r
                break
        if perf_hdr_row is not None:
            for dr in range(perf_hdr_row + 1, perf_hdr_row + 30):
...
with st.sidebar:
    ...
    if uploaded_files:
        # Let user pick which file to view
        file_names = [f.name for f in uploaded_files]
        if len(file_names) > 1:
            selected_file_name = st.selectbox("📁 Select File", file_names)
        else:
            selected_file_name = file_names[0]

        uploaded_file = next(f for f in uploaded_files if f.name == selected_file_name)

        # Derive display title from filename (strip extension)
    ...
    st.markdown("---")
    st.markdown("<small style='color:#334155;font-size:.68rem'>Universal Sales Hub · v11.0</small>",
                unsafe_allow_html=True)
...
if not uploaded_files:
    ...
    <div class="landing">
        ...
        <p>Upload <strong>any Excel file</strong> using the same sales format to instantly see
        targets, achievements and the DM → RP hierarchy — all figures read directly from your Excel.</p>
        <div class="landing-card">
            <h4>✅ Works with any Excel file sharing this format</h4>
            <ul>
                <li><strong>Sheets:</strong> APR, MAY, JUN … MAR (monthly data)</li>
                <li><strong>Row 0:</strong> Entity names (TOTAL, DMs with "(DM)", RPs)</li>
                <li><strong>Row 1:</strong> TAR / ACH headers per entity</li>
                <li><strong>Rows 2+:</strong> Products — dynamically detected</li>
                <li><strong>TOTAL row:</strong> Auto-detected regardless of how many products</li>
                <li><strong>Multiple files:</strong> Upload MEGA DERMA + HIMALAYA together, switch between them</li>
...
# ══════════════════════════════════════════════════════
# PAGE HEADER — uses dynamic title from filename
# ══════════════════════════════════════════════════════
...
with tab1:
    ...
    numeric_cols = ["Target LKR","Achievement LKR","LKR Variance","Target Units","Achievement Units","Unit Variance"]
    ...
    st.dataframe(
        ...
        .map(lambda v:("color:#059669;font-weight:700" if v>=0 else "color:#dc2626;font-weight:700") if isinstance(v,(int,float)) else "",
             subset=["LKR Variance","Unit Variance"])
        .map(lambda v:("color:#059669;font-weight:700" if v>=100 else "color:#d97706;font-weight:700" if v>=80 else "color:#dc2626;font-weight:700") if isinstance(v,(int,float)) else "",
...
with tab2:
    ...
    kpi_card(g3,"Overall LKR Ach %",f"{grand_pct_lkr:.1f}%","📈",
             "c-green" if grand_pct_lkr>=100 else "c-amber", badge_cls=pct_cls(grand_pct_lkr))
    kpi_card(g4,"LKR Variance (total)",fmt_lkr(abs(grand_var_lkr)),
             "📊" if grand_var_lkr>=0 else "📉","c-teal" if grand_var_lkr>=0 else "c-red",
    ...
    styled_s = (sum_df.style
        ...
        .map(lambda v:("color:#16a34a;font-weight:700" if v>=100 else "color:#d97706;font-weight:700" if v>=80 else "color:#dc2626;font-weight:700") if isinstance(v,(int,float)) else "",
             subset=["LKR Ach %","Unit Ach %"]))
...
with tab3:
        ...
        for i, dm_name in enumerate(active_dms):
            ...
            dm_cols[i].markdown(f"""
                <div style="background:#fff;border:1px solid #e8edf5;border-radius:14px;
                  ...
                  <div style="font-size:1rem;font-weight:700;color:{bar_color}">{fmt_lkr(dm_a)}</div>
                  <div style="background:#f1f5f9;border-radius:999px;height:6px;margin-bottom:8px">
                    <div style="width:{ach_w:.1f}%;height:100%;background:{bar_color};border-radius:999px"></div>
                  </div>
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    <div style="font-size:0.7rem;font-weight:600;color:{var_color}">{var_sign}{fmt_lkr(dm_v)}</div>
    ...
    if dm_trend:
        ...
        st.markdown('<div class="card-title">Monthly ACH vs Target Trend</div>', unsafe_allow_html=True)
...
with tab4:
    ...
    if not disp_reps:
        ...
        for owner, rps in groups.items():
            ...
            html = f"""
             <div style="border:1px solid #e2e8f0;border-radius:16px;overflow:hidden;
                 ...
                 <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
                   ...
                   <div style="display:flex;gap:0;border:1px solid rgba(255,255,255,0.1);border-radius:10px;overflow:hidden">
                     <div style="padding:0.6rem 1.1rem;border-right:1px solid rgba(255,255,255,0.1)">
                       <div style="font-size:0.58rem;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.09em;margin-bottom:3px">Target</div>
                       <div style="font-size:0.95rem;font-weight:700;color:#cbd5e1;white-space:nowrap">{fmt_n(int(dm_t))}</div>
                     </div>
                     <div style="padding:0.6rem 1.1rem;border-right:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.04)">
                       <div style="font-size:0.58rem;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.09em;margin-bottom:3px">Achievement</div>
                       <div style="font-size:0.95rem;font-weight:700;color:{dm_bc};white-space:nowrap">{fmt_n(int(dm_a))}</div>
                     </div>
                     <div style="padding:0.6rem 1.1rem;border-right:1px solid rgba(255,255,255,0.1)">
                       <div style="font-size:0.58rem;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.09em;margin-bottom:3px">Variance</div>
                       <div style="font-size:0.95rem;font-weight:700;color:{dm_bc};white-space:nowrap">{"+" if dm_v>=0 else ""}{fmt_n(int(dm_v))}</div>
                     </div>
                     <div style="padding:0.6rem 1.1rem;display:flex;align-items:center">
                       <span style="font-size:0.9rem;font-weight:800;padding:4px 14px;border-radius:8px;background:{dm_bg};color:{dm_fg};white-space:nowrap">{dm_p:.1f}%</span>
                     </div>
                   </div>
                 </div>
               ...
               <div style="display:flex;align-items:center;gap:0;padding:7px 14px;background:#f8fafc;border-bottom:2px solid #e2e8f0">
                 ...
                 <div style="width:140px;text-align:right;font-size:0.6rem;font-weight:700;color:#64748b;text-transform:uppercase">🎯 Target</div>
                 <div style="width:140px;text-align:right;font-size:0.6rem;font-weight:700;color:#64748b;text-transform:uppercase">✅ Achievement</div>
                 <div style="width:110px;text-align:right;font-size:0.6rem;font-weight:700;color:#64748b;text-transform:uppercase">Variance</div>
                 <div style="flex:1.2;padding:0 12px;font-size:0.6rem;font-weight:700;color:#94a3b8;text-transform:uppercase">Progress</div>
                 <div style="width:62px;text-align:center;font-size:0.6rem;font-weight:700;color:#94a3b8;text-transform:uppercase">Ach %</div>
               </div>"""

            for i, r in enumerate(rp_rows_s):
                p = r['PCT']; c = pct_color(p); var = r['VAR']; vc = "#059669" if var>=0 else "#dc2626"
                ...
                html += f"""
               <div style="display:flex;align-items:center;gap:0;padding:11px 14px;background:{bg};border-bottom:1px solid #f1f5f9">
                 <div style="width:28px;font-size:0.68rem;font-weight:700;color:#94a3b8">#{i+1}</div>
                 <div style="flex:1.8;font-size:0.85rem;font-weight:600;color:#1e293b;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-right:10px">{r['name']}</div>
                 <div style="width:140px;text-align:right;padding-right:6px">
                   <div style="font-size:0.65rem;color:#94a3b8;font-weight:600;text-transform:uppercase;margin-bottom:1px">Target</div>
                   <div style="font-size:0.92rem;font-weight:700;color:#334155">{fmt_n(int(r['TAR']))}</div>
                 </div>
                 <div style="width:140px;text-align:right;padding-right:6px">
                   <div style="font-size:0.65rem;color:#94a3b8;font-weight:600;text-transform:uppercase;margin-bottom:1px">Achievement</div>
                   <div style="font-size:0.92rem;font-weight:800;color:{c}">{fmt_n(int(r['ACH']))}</div>
                 </div>
                 <div style="width:110px;text-align:right;padding-right:6px">
                   <div style="font-size:0.65rem;color:#94a3b8;font-weight:600;text-transform:uppercase;margin-bottom:1px">Variance</div>
                   <div style="font-size:0.88rem;font-weight:700;color:{vc}">{"+" if var>=0 else ""}{fmt_n(int(var))}</div>
                 </div>
                 <div style="flex:1.2;padding:0 12px">
                   <div style="background:#e8edf5;border-radius:999px;height:9px;overflow:hidden">
                     <div style="width:{min(p,100):.1f}%;height:100%;background:{c};border-radius:999px;opacity:0.85"></div>
                   </div>
                 </div>
                 <div style="width:62px;text-align:center">
                   <span style="font-size:0.75rem;font-weight:800;padding:3px 8px;border-radius:7px;background:{pbg};color:{pfg}">{p:.1f}%</span>
                 </div>
               </div>"""
            html += "</div>"
            st.markdown(html, unsafe_allow_html=True)
        ...
        rp_tbl = [{"Rep":r,"Under DM":rp_to_dm.get(r,"—"),
                   ...
                   "Unit Ach%":round(rp_units_ms.get(r,{}).get('PCT',0),1)} for r in disp_reps]
        if rp_tbl:
...
with tab6:
    ...
    if trend_rows:
        ...
        full_tbl = tr_df.rename(columns={"TAR_LKR":"Target (LKR)","ACH_LKR":"Achievement (LKR)","PCT_LKR":"Ach %","VAR_LKR":"Variance (LKR)"})
...
with tab7:
    ...
    <strong>Cumulative % = rolling ACH ÷ rolling TAR from the first month onwards.</strong>
    Each month adds to the running total so the % reflects true year-to-date performance.
    </div>""", unsafe_allow_html=True)
    ...
    show_entities = ['TOTAL'] if cum_sel_entity == "ALL (TOTAL only)" else [cum_sel_entity]
    ...
    if cum_chart_rows:
        ...
        with col_line:
            fig_cum_line = px.line(cc_df, x="Month", y="CUM_PCT", color="Product", markers=True,
            ...
            fig_cum_line.update_layout(**PLOTLY_BASE, height=360, margin=dict(t=10,b=40,l=70,r=20),
                legend=dict(bgcolor="rgba(0,0,0,0)",orientation="h",y=1.05,xanchor="right",x=1,font=dict(size=10)),
                xaxis=dict(tickfont=dict(size=11,color="#64748b"),showgrid=False),
                yaxis=dict(gridcolor="#f1f5f9",tickfont=dict(size=10,color="#94a3b8"),title="Cumulative Ach %",ticksuffix="%"))
        ...
        with col_bar_cum:
            ...
            if not last_month_df.empty:
                fig_cum_bar = go.Figure(go.Bar(
                    y=last_month_df["Product"], x=last_month_df["CUM_PCT"], orientation="h",
                    marker=dict(color=[pct_color(p) for p in last_month_df["CUM_PCT"]],opacity=0.85,line=dict(width=0)),
                    text=[f"{p:.1f}%" for p in last_month_df["CUM_PCT"]], textposition="outside"))
                fig_cum_bar.add_vline(x=100, line_color="#22c55e", line_dash="dot", line_width=1.5)
                fig_cum_bar.update_layout(**PLOTLY_BASE, height=360, margin=dict(t=10,b=40,l=160,r=70),
                    xaxis=dict(gridcolor="#f1f5f9",tickfont=dict(size=10,color="#94a3b8"),ticksuffix="%"),
                    yaxis=dict(tickfont=dict(size=10,color="#64748b")))
    ...
    for entity in all_entities:
        ...
        prods_for_comp = all_products if comp_product == "ALL Products" else [comp_product]
        c_tar = sum(cum_data.get(cum_sel_month,{}).get(entity,{}).get(p,{}).get('CUM_TAR',0.0) for p in prods_for_comp)
        c_ach = sum(cum_data.get(cum_sel_month,{}).get(entity,{}).get(p,{}).get('CUM_ACH',0.0) for p in prods_for_comp)
    ...
    if comp_rows:
        ...
        comp_display = comp_df.rename(columns={
            "CUM_TAR":"Cum Target (units)", "CUM_ACH":"Cum Achievement (units)",
            "CUM_PCT":"Cum Ach %", "CUM_VAR":"Cum Variance (units)"})
        st.dataframe(comp_display.style.format({
            "Cum Target (units)":"{:,.0f}", "Cum Achievement (units)":"{:,.0f}",
            "Cum Ach %":"{:.1f}%", "Cum Variance (units)":"{:+,.0f}"}),
            use_container_width=True, hide_index=True, height=min(500,len(comp_display)*42+60))
...
# ══════════════════════════════════════════════════════
st.markdown(
    f'<div class="dash-footer">Universal Sales Intelligence Hub · {dash_title} · {sel_month} · v11.0</div>',
    unsafe_allow_html=True)
